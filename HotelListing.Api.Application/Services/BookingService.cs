...
public class BookingService(HotelListingDbContext context, IUsersService usersService, IMapper mapper, IHotelsService hotelsService) : IBookingService
{
    ...
    {
        var hotelExists = await hotelsService.HotelExistsAsync(hotelId); // This hotel existence check is only necessary for System Administrators.
                                                                         //ghyrt hon                                                      // HotelAdmins are guaranteed by the FK constraint to be linked to a valid Hotel.
        if (!hotelExists)
        {
            return Result<PagedResult<GetBookingDto>>.NotFound(new Error(ErrorCodes.NotFound, $"Hotel with id '{hotelId}' was not found."));
        }
    ...
    public async Task<Result<GetBookingDto>> CreateBookingAsync(CreateBookingDto dto, int hotelId)
    {
        ...
        //ghyrt
        var hotel = await hotelsService.HotelObjectExistsAsync(hotelId);
        if (hotel == null)
        {
    ...
    {
        var userId = usersService.UserId; //ghyr hon
        var hotelExists = await hotelsService.HotelExistsAsync(hotelId);
        if (!hotelExists)
        {
            return Result<PagedResult<GetBookingDto>>.NotFound(new Error(ErrorCodes.NotFound, $"Hotel with id '{hotelId}' was not found."));
        }
